<template>
    <AdminLayout>
        <div class="overflow-x-auto" >
           ...
           <div class="flex mt-4 mr-20 justify-end">
            <router-link :to="{name : 'approvedorder'}">
                <button name="approved" class="ml-20 bg-purple-200 dark:bg-violet-900 m-4 mr-8 text-3xl text-purple-900 dark:text-purple-200  
                  ...
                  <div class="bg-purple-300  dark:bg-purple-800 shadow-md rounded my-6">
                      <table class="min-w-max w-full table-auto rounded-lg" >
                          ...
                          <tbody class="text-purple-900 dark:text-gray-200 text-md font-light" 
                            :key="order.id" v-for="order in allorders" >
                              <tr class="border-b border-gray-200 hover:bg-purple-500">
                                  ...
                                  <td class="py-3 px-6 text-center">
                                      <div class="flex item-center justify-center">
                                          ...
                                          <button :key="order.id"
                                              @click="approve(order)"  class="text-xs text-white bg-sky-600 rounded-full w-14 mr-2 transform
...
export default {
    ...
    methods:{
        ...mapActions('order',['ordered','approve']),
        appr(order){
            //console.log(id)
            this.approve(order)
            .then(response => {
                this.$router.push({ name: 'indexorder' })
                this.successMessage="order Approve Successfully!"
            })
            .catch(error => {
                console.log(error)
    ...
    computed:{
    ...mapGetters('order', {allorders: "allorders"}),     
    }
