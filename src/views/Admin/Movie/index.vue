                          ...
                          <!-- // v-for="(post, index) in posts" :key="index"> -->
                            <tr class="border-b border-gray-200 hover:bg-purple-500">
                                ...
                                <td class="py-3 px-6 text-center">
                                    <div class="flex items-center justify-center" v-for="genre in movie.genres" :key="genre.id">
                                           {{genre.name}}
                                    </div>
                                </td>
                                <td class="py-3 px-6 text-center">
                                ...
                                <td class="py-3 px-6 text-center">
                                    <div class="flex item-center justify-center">
                                        ...
                                        <router-link :to="{ name: 'editmovie', params:{movieid: movie.id}}">
                                            ...
                                            <button :key="movie.id" @click="destroy(movie.id)" class="w-6 mr-2 transform hover:text-purple-300 hover:scale-110">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<script>
  ...
  <!-- methods:mapActions('auth',['deleteToken']),
  created() {
    this.deleteToken()
      .then(response => {
          console.log(response)
