<template>
    <div class="flex justify-between" style="margin-left:20px;margin-right:30px; margin-bottom:20px;">
        <div class="mt-20" v-for="movie in shmovies.data" :key="movie.id"> 
            <div class=" max-w-sm mx-20 shadow-2xl mt-10 w-60 rounded-lg overflow-hidden shadow-lg" style="background-color: #d1b3ff;">
                <router-link to="/">
                <img class=" w-full" style="height:255px;width:200px;" alt="image" :src="`http://127.0.0.1:8000/${movie.image}`" />
                <div class="px-6 py-1">
                <div class="font-bold text-2xl text-gray-900 mb-2">
                    {{movie.name}}
                </div>
                    <p class="px-2 font-semibold text-gray-700 text-m">
                        {{movie.type}}
                    </p>
                </div>

                <div class="px-6 pt-1 pb-2">
                <span class="py-2 inline-block bg-gray-300 rounded-full px-2 py-1 text-sm font-semibold text-gray-800 mr-2 mb-2">
                    {{movie.from}} /{{movie.to}}
                </span>
                
                <span class="mt-2 inline-block bg-gray-300 rounded-full px-2 py-1 text-m font-semibold 
                  text-gray-800 mr-2 mb-2" v-for="genre in movie.genres" :key="genre.id">
                          {{genre.name}}                   
                </span>
                </div>
                </router-link>
